...
class DiscoverModule(Discover):
    def __init__(self):
        super().__init__()
        self.description = '检测目标站点使用的MID'

    def start_up(self, target):
        with open(os.path.join('wordlist', Global.Options['mid_dict']), 'r', encoding='utf-8') as f:
            mid_list = json.load(f)
        with ThreadPoolExecutor(max_workers=Global.Options['threads']) as executor:
            result_list = executor.map(self.thread_task, [[target, _] for _ in mid_list])
        # count = len(set(list(result_list)))
        # result_list = ['{}:{:.0%}'.format(_[0], _[1] / count) for _ in Counter(list(set(list(result_list)))).most_common(count)]
        return 'web mid', list(filter(lambda x: x is not None, result_list))
